...
type syncMemBatch struct {
	batch map[types.Hash32][]byte // In-memory membatch of recently completed items
	order []types.Hash32          // Order of completion to prevent out-of-order data loss
}

// newSyncMemBatch allocates a new memory-buffer for not-yet persisted trie nodes.
func newSyncMemBatch() *syncMemBatch {
	return &syncMemBatch{
		batch: make(map[types.Hash32][]byte),
		order: make([]types.Hash32, 0, 256),
	}
}
...
func NewSync(root types.Hash32, database DatabaseReader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
		membatch: newSyncMemBatch(),
		requests: make(map[types.Hash32]*request),
		queue:    prque.New(nil),
	}
	ts.AddSubTrie(root, 0, types.Hash32{}, callback)
	return ts
}

// AddSubTrie registers a new trie to the sync code, rooted at the designated parent.
func (s *Sync) AddSubTrie(root types.Hash32, depth int, parent types.Hash32, callback LeafCallback) {
...
// contract code).
func (s *Sync) AddRawEntry(hash types.Hash32, depth int, parent types.Hash32) {
...
func (s *Sync) Missing(max int) []types.Hash32 {
	requests := []types.Hash32{}
	for !s.queue.Empty() && (max == 0 || len(requests) < max) {
		requests = append(requests, s.queue.PopItem().(types.Hash32))
	}
	return requests
}
