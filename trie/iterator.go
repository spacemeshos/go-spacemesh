...
func (it *nodeIterator) Hash() types.Hash32 {
	if len(it.stack) == 0 {
		return types.Hash32{}
	}
...
func (it *nodeIterator) Parent() types.Hash32 {
	if len(it.stack) == 0 {
		return types.Hash32{}
	}
...
func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor types.Hash32) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() types.Hash32 {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() types.Hash32 {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash() == types.Hash32{}
...
func (it *unionIterator) Hash() types.Hash32 {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() types.Hash32 {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == types.Hash32{}) {
			it.count++
