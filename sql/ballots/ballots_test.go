...
func TestHas(t *testing.T) {
	db := sql.InMemory()
	ballot := types.NewExistingBallot(types.BallotID{1}, types.EmptyEdSignature, types.NodeID{}, types.BallotMetadata{})
...
func TestLatest(t *testing.T) {
	...
	ballot := types.NewExistingBallot(types.BallotID{1}, types.EmptyEdSignature, types.NodeID{}, types.BallotMetadata{Layer: types.NewLayerID(11)})
	require.NoError(t, Add(db, &ballot))
	latest, err = LatestLayer(db)
	require.NoError(t, err)
	require.Equal(t, ballot.Layer, latest)

	newBallot := types.NewExistingBallot(types.BallotID{2}, types.EmptyEdSignature, types.NodeID{}, types.BallotMetadata{Layer: types.NewLayerID(12)})
