...
func (h *Handler) HandleSyncedMalfeasanceProof(ctx context.Context, _ p2p.Peer, data []byte) error {
	...
	return validateAndSave(ctx, h.logger, h.cdb, h.edVerifier, h.tortoise, &types.MalfeasanceGossip{MalfeasanceProof: p})
}
...
func (h *Handler) HandleMalfeasanceProof(ctx context.Context, peer p2p.Peer, data []byte) error {
	...
	return validateAndSave(ctx, h.logger, h.cdb, h.edVerifier, h.tortoise, &p)
}
...
func validateAndSave(
	...
	nodeID, err := Validate(ctx, logger, cdb, edVerifier, p)
	if err != nil {
		return err
	}
