...
Node and account keys are not multi-hashes. However their binary and string representation````********```` also include a type and so will support forward-compatible changes and key type and data size.
