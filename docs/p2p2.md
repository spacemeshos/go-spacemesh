...
- Keys system is secp256k1 as it includes encrypt/decrypt (unlike ed25519 which only supports signing)
- We use base58 encoding for string rep of keys (human readable)
- In proto messages keys/ids can be []bytes - no need for encoding as we are using a binary p2p format - greatly simplifies everything.
...
### Wire message format
```
<64 bits data size - big endian><message binary data (protobufs-bin-encoded)>
```

- We need to use length prefixed protobufs messages as protobufs data doesn't include length to allow multiple messages on the same connection.
...
```
data {
    sessionId: uuid-string      // if there's a session between the peers, empty or missing otherwise
    senderPubKey: []byte        // sender id to decrypt handshake protocol messages. empty for session messages.
    message: []byte <encrypted-binary-protobufs format> // encrypted payload eighter w server key or with senders noode privkey and receiver node pub key (secp256k1 enc)
}
````
