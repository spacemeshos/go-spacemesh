...
func (mbn *mockBaseNetwork) RegisterGossipProtocol(protocol string, prio int) chan service.GossipMessage {
...
func (pq *mockPQ) Set(name string, prio int, bufferSize int) error {
	return nil
}

func (pq *mockPQ) Write(name string, m interface{}) error {
	pq.q <- m
	return nil
}
...
func TestNeighborhood_AddIncomingPeer(t *testing.T) {
	n := NewProtocol(config.DefaultConfig().SwarmConfig, newMockBaseNetwork(), newPubkey(t), newMockPQ(), log.New(t.Name(), "", ""))
	n.Start()
	pub := p2pcrypto.NewRandomPubkey()
	n.addPeer(pub)
...
func TestNeighborhood_Relay(t *testing.T) {
	...
	_ = net.RegisterGossipProtocol("Someproto", 0)
	n := NewProtocol(config.DefaultConfig().SwarmConfig, net, newPubkey(t), newMockPQ(), log.New(t.Name(), "", ""))
	n.Start()
...
func TestNeighborhood_Broadcast3(t *testing.T) {
	...
	net := newMockBaseNetwork()
	pk := p2pcrypto.NewRandomPubkey()
	n := NewProtocol(config.DefaultConfig().SwarmConfig, net, pk, newMockPQ(), log.New(t.Name(), "", ""))
	n.Start()
