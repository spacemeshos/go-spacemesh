...
func TestFindNodeProtocol_FindNode(t *testing.T) {
	...
	fnd1 := newFindNodeProtocol(n1, rt1, cfg.SwarmConfig)

	n2 := sim.NewNode()
	rt2 := NewRoutingTable(cfg.SwarmConfig.RoutingTableBucketSize, n2.DhtID(), getTestLogger("FindNode - ").Logger)
	_ = newFindNodeProtocol(n2, rt2, cfg.SwarmConfig)

	idarr, err := fnd1.FindNode(n2.Node, node.GenerateRandomNodeData().String(), cfg.SwarmConfig)
...
func TestFindNodeProtocol_FindNode2(t *testing.T) {
	...
	n2 := sim.NewNode()
	rt2 := NewRoutingTable(cfg.SwarmConfig.RoutingTableBucketSize, n2.DhtID(), getTestLogger("FindNode - ").Logger)
	fnd2 := newFindNodeProtocol(n2, rt2, cfg.SwarmConfig)

	fnd2.rt.Update(randnode)

	idarr, err := fnd1.FindNode(n2.Node, randnode.String(), cfg.SwarmConfig)
	...
	idarr, err = fnd2.FindNode(n1.Node, randnode.String(), cfg.SwarmConfig)
