...
func TestFindNodeProtocol_FindNode(t *testing.T) {
	...
	n1 := sim.NewNode()
	rt1 := NewRoutingTable(cfg.DiscoveryConfig.RoutingTableBucketSize, n1.DhtID(), getTestLogger("FindNode - ").Logger)
	fnd1 := newFindNodeProtocol(n1, rt1)

	n2 := sim.NewNode()
	rt2 := NewRoutingTable(cfg.DiscoveryConfig.RoutingTableBucketSize, n2.DhtID(), getTestLogger("FindNode - ").Logger)
