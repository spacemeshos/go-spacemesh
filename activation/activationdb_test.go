...
func getAtxDb(id string) (*DB, *mesh.Mesh, database.Database) {
	...
	atxdb := NewDB(atxStore, NewIdentityStore(database.NewMemDatabase()), nil, memesh, layersPerEpochBig, &ValidatorMock{}, lg.WithName("atxDB"))
...
func TestActivationDb_CalcActiveSetFromViewWithCache(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		go func() {
			num, err := atxdb.CalcActiveSetFromView(atx.View, atx.PubLayerID.GetEpoch())
			assert.NoError(t, err)
...
func Test_CalcActiveSetFromView(t *testing.T) {
	...
	num, err = atxdb.CalcActiveSetFromView(atx2.View, atx2.PubLayerID.GetEpoch())
	assert.NoError(t, err)
	assert.Equal(t, 3, int(num))
...
func BenchmarkNewActivationDb(b *testing.B) {
	...
	r.NoError(err)
	atxdb := NewDB(store, NewIdentityStore(store), nil, msh, layersPerEpochBig, &ValidatorMock{}, lg.WithName("atxDB"))
