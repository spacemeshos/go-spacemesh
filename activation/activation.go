...
func (b *Builder) loop() {
	...
	for {
		...
		if err := b.PublishActivationTx(); err != nil {
			...
			events.Publish(events.AtxCreated{Created: false, Layer: uint32(b.currentEpoch())})
...
func (b *Builder) PublishActivationTx() error {
	...
	events.Publish(events.AtxCreated{Created: true, ID: atx.ShortString(), Layer: uint32(b.currentEpoch())})
