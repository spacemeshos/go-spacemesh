...
func newBuilder(activationDb atxDBProvider) *Builder {
	...
	b := NewBuilder(cfg, nodeID, 0, &MockSigning{}, activationDb, net, meshProviderMock, layersPerEpoch, nipstBuilderMock, postProver, layerClockMock, &mockSyncer{}, NewMockDB(), lg.WithName("atxBuilder"))
...
func TestBuilder_PublishActivationTx_RebuildNIPoSTWhenTargetEpochPassed(t *testing.T) {
	...
	// create and attempt to publish ATX
	faultyNet := &FaultyNetMock{retErr: true}
	b := NewBuilder(cfg, nodeID, 0, &MockSigning{}, activationDb, faultyNet, meshProviderMock, layersPerEpoch, nipstBuilderMock, postProver, layerClockMock, &mockSyncer{}, NewMockDB(), lg.WithName("atxBuilder"))
	published, builtNIPoST, err := publishAtx(b, postGenesisEpochLayer+1, postGenesisEpoch, layersPerEpoch)
	r.EqualError(err, "failed to broadcast ATX: faulty")
	r.False(published)
	r.True(builtNIPoST)
...
func TestBuilder_PublishActivationTx_FailsWhenNIPoSTBuilderFails(t *testing.T) {
	...
	b := NewBuilder(cfg, nodeID, 0, &MockSigning{}, activationDb, net, meshProviderMock, layersPerEpoch, nipostBuilder, postProver, layerClockMock, &mockSyncer{}, NewMockDB(), lg.WithName("atxBuilder"))
	b.initialPoST = initialPoST
...
func TestBuilder_PublishActivationTx_Serialize(t *testing.T) {
	...
	atx := newActivationTx(nodeID, 1, prevAtxID, 5, 1, prevAtxID, coinbase, defaultTotalWeight, []types.BlockID{block1.ID(), block2.ID(), block3.ID()}, nipost)
	storeAtx(r, activationDb, atx, log.NewDefault("storeAtx"))

	// MERGE FIX
	// 	act := newActivationTx(b.nodeID, 2, atx.ID(), atx.ID(), atx.PubLayerID+10, 0, 100, 100, coinbase, npst) -- THEIR
	// 	act := newActivationTx(b.nodeID, 2, atx.ID(), atx.PubLayerID+10, 0, atx.ID(), coinbase, defaultActiveSetSize, view, nipost) -- MINE
	act := newActivationTx(b.nodeID, 2, atx.ID(), atx.PubLayerID+10, 0, atx.ID(), 0, 100, 100, coinbase, defaultTotalWeight, nipost)
...
func TestBuilder_SignAtx(t *testing.T) {
	...
	b := NewBuilder(cfg, nodeID, 0, ed, activationDb, net, meshProviderMock, layersPerEpoch, nipstBuilderMock, postProver, layerClockMock, &mockSyncer{}, NewMockDB(), lg.WithName("atxBuilder"))
...
func TestBuilder_NIPoSTPublishRecovery(t *testing.T) {
	...
	b := NewBuilder(cfg, id, 0, sig, activationDb, &FaultyNetMock{}, layers, layersPerEpoch, nipstBuilder, postProver, layerClockMock, &mockSyncer{}, db, lg.WithName("atxBuilder"))
	...
	b = NewBuilder(cfg, id, 0, &MockSigning{}, activationDb, &FaultyNetMock{}, layers, layersPerEpoch, nipostBuilder, postProver, layerClockMock, &mockSyncer{}, db, lg.WithName("atxBuilder"))
	err = b.buildNIPoSTChallenge(0)
	assert.NoError(t, err)
	db.hadNone = false
	// test load challenge in later epoch - NIPoST should be truncated
	b = NewBuilder(cfg, id, 0, &MockSigning{}, activationDb, &FaultyNetMock{}, layers, layersPerEpoch, nipstBuilder, postProver, layerClockMock, &mockSyncer{}, db, lg.WithName("atxBuilder"))
	err = b.loadChallenge()
	assert.NoError(t, err)
...
func TestActivationDb_CalcActiveSetFromViewHighConcurrency(t *testing.T) {
	...
	atxs := []*types.ActivationTx{
		...
		newActivationTx(id1, 0, *types.EmptyATXID, *types.EmptyATXID, 12, 0, 100, 100, coinbase1, &types.NIPoST{}),
		newActivationTx(id2, 0, *types.EmptyATXID, *types.EmptyATXID, 300, 0, 100, 100, coinbase2, &types.NIPoST{}),
		newActivationTx(id3, 0, *types.EmptyATXID, *types.EmptyATXID, 435, 0, 100, 100, coinbase3, &types.NIPoST{}),
	}
	...
	atx := newActivationTx(id1, 1, atxs[0].ID(), atxs[0].ID(), 1000, 0, 100, 100, coinbase1, &types.NIPoST{})
