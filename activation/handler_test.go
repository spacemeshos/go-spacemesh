...
func TestHandler_processBlockATXs(t *testing.T) {
	...
	atxList := []*types.VerifiedActivationTx{
		newActivationTx(
			...
			coinbase1,
			numUnits,
			types.NIPost{PostMetadata: types.PostMetadata{Challenge: poetRef}},
		),
		newActivationTx(
			t,
			sig2,
...
func TestHandler_ContextuallyValidateAtx(t *testing.T) {
	...
	t.Run("valid atx", func(t *testing.T) {
		...
		atx := newAtx(challenge, types.NIPost{}, 2, types.Address{})
		require.NoError(t, SignAndFinalizeAtx(sig, atx))
...
func testHandler_PostMalfeasanceProofs(t *testing.T, synced bool) {
	...
	if !synced {
		atxHdlr.mpub.EXPECT().Publish(gomock.Any(), pubsub.MalfeasanceProof, gomock.Any()).DoAndReturn(
			func(_ context.Context, _ string, data []byte) error {
				...
				p, ok := got.Proof.Data.(*wire.InvalidPostIndexProofV1)
				require.True(t, ok)
	...
	msg := codec.MustEncode(atx.ToWireV1())
	if synced {
		require.NoError(
...
func testHandler_HandleMaliciousAtx(t *testing.T, synced bool) {
	...
	atxHdlr.mValidator.EXPECT().
		Post(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any())
	msg := codec.MustEncode(atx1.ToWireV1())
	...
	msg = codec.MustEncode(atx2.ToWireV1())
	if synced {
		require.NoError(
	...
	for i := 0; i < runtime.NumCPU()/2; i++ {
		...
		go func() {
			...
			for i := 0; ; i++ {
				...
				sig, err := signing.NewEdSigner()
				require.NoError(b, err)
				atx := newAtx(challenge, types.NIPost{}, 1, types.Address{})
...
func TestHandler_AtxWeight(t *testing.T) {
	...
	buf, err := codec.Encode(atx1.ToWireV1())
	require.NoError(t, err)
	...
	buf, err = codec.Encode(atx2.ToWireV1())
	require.NoError(t, err)
