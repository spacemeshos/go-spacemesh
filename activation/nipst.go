...
//	// operation.
//	Initialize(computeProviderId uint) error
//
...
//	Reset() error
//
//	DiskState() (*initialization.DiskState, error)
//
...
//
//	// SetParams updates the datadir and numLabels params in the client config, to be used in the initialization and the
//	// execution phases. It overrides the config which the client was instantiated with.
//	SetParams(datadir string, numLabels uint64) error
//
...
func (nb *NIPSTBuilder) BuildNIPST(challenge *types.Hash32, atxExpired, stop chan struct{}) (*types.NIPST, error) {
	...
	if nipst.PostProof == nil {
		...
		startTime := time.Now()
		proof, err := nb.post.GenerateProof(nb.state.PoetProofRef)
		if err != nil {
