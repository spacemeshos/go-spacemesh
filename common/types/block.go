...
type InnerBlock struct {
	LayerIndex LayerID
	TickHeight uint64
	Rewards    []AnyReward     `scale:"max=1000"` // TODO(mafa): check if this is the right limit
	TxIDs      []TransactionID `scale:"max=1000"` // TODO(mafa): check if this is the right limit
}
...
type Certificate struct {
	BlockID    BlockID
	Signatures []CertifyMessage `scale:"max=100"` // TODO(mafa): check if this is the right limit
}
...
type CertifyContent struct {
	...
	Proof []byte `scale:"max=32"` // TODO(mafa): check max size
}
