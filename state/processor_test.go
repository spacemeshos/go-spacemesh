...
func (s *ProcessorStateSuite) TestTransactionProcessor_ApplyTransaction() {
	...
	failed, err := s.processor.ApplyTransactions(types.LayerIDFromUint32(1), transactions)
	assert.NoError(s.T(), err)
...
func (s *ProcessorStateSuite) TestTransactionProcessor_ApplyRewards() {
	s.processor.ApplyRewards(types.LayerIDFromUint32(1), []types.Address{types.HexToAddress("aaa"),
...
func (s *ProcessorStateSuite) TestTransactionProcessor_ApplyTransaction_OrderByNonce() {
	...
	s.processor.ApplyTransactions(types.LayerIDFromUint32(1), transactions)
	//assert.Error(s.T(), err)
...
func (s *ProcessorStateSuite) TestTransactionProcessor_Reset() {
	...
	failed, err := processor.ApplyTransactions(types.LayerIDFromUint32(1), transactions)
	assert.NoError(s.T(), err)
	assert.True(s.T(), failed == 0)
	...
	failed, err = processor.ApplyTransactions(types.LayerIDFromUint32(2), transactions)
	assert.True(s.T(), failed == 0)
	assert.NoError(s.T(), err)
	...
	err = processor.LoadState(types.LayerIDFromUint32(1))
	assert.NoError(s.T(), err)
...
func (s *ProcessorStateSuite) TestTransactionProcessor_Multilayer() {
	...
	for i := 0; i < testCycles; i++ {
		...
		failed, err := processor.ApplyTransactions(types.LayerIDFromUint32(uint32(i)), trns)
		assert.NoError(s.T(), err)
		assert.True(s.T(), failed == 0)
		...
		if i == revertToLayer+revertAfterLayer {
			err = processor.LoadState(types.LayerIDFromUint32(uint32(revertToLayer)))
			assert.NoError(s.T(), err)
			got := string(processor.Dump())
...
func TestTransactionProcessor_GetStateRoot(t *testing.T) {
	...
	r.NoError(proc.addState(expectedRoot, types.LayerIDFromUint32(1)))
...
func TestTransactionProcessor_ApplyTransactions(t *testing.T) {
	...
	_, err = processor.ApplyTransactions(types.LayerIDFromUint32(1), transactions)
	assert.NoError(t, err)

	_, err = processor.ApplyTransactions(types.LayerIDFromUint32(2), []*types.Transaction{})
	assert.NoError(t, err)

	_, err = processor.ApplyTransactions(types.LayerIDFromUint32(3), []*types.Transaction{})
	assert.NoError(t, err)

	_, err = processor.GetLayerStateRoot(types.LayerIDFromUint32(3))
	assert.NoError(t, err)
