...
func TestRemoteHare(t *testing.T) {
	...
	require.NoError(t, hare.run(t.Context(), s))
}

func TestRemoteHare_NotActiveInPreround(t *testing.T) {
	cfg := Config{
		Committee:       10,
		IterationsLimit: 5,
		PreroundDelay:   time.Millisecond,
...
func TestRemoteHare_NotActive(t *testing.T) {
	cfg := Config{
		Committee:       10,
		IterationsLimit: 5,
		PreroundDelay:   time.Millisecond,
	...
	// Not active in any round
	oracle.EXPECT().
		CalcEligibility(gomock.Any(), s.lid, gomock.Any(), gomock.Any(), signer.NodeID(), gomock.Any()).
		Return(0, nil).
		Times(7)

	// Still certifies the block
	block := types.BlockID{9, 8, 7, 6}
	svc.EXPECT().BlockID(gomock.Any(), s.lid).Return(block, nil)
	certifier.EXPECT().CertifyBlock(gomock.Any(), signer, s.lid, block, s.beacon).Return(nil)
...
func TestRemoteHare_NotConvergeStillTerminates(t *testing.T) {
	cfg := Config{
		Committee:       10,
		IterationsLimit: 5,
		PreroundDelay:   time.Millisecond,
	...
	// Hare checks for block ID every 1+th iteration, in every round.
	expectedTimes := int(cfg.IterationsLimit-1)*7 + 1
	svc.EXPECT().BlockID(gomock.Any(), s.lid).Return(types.EmptyBlockID, nil).Times(expectedTimes)
	err = hare.run(t.Context(), s)
	require.ErrorContains(t, err, "failed to reach consensus")
}
