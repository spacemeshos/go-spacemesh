...
type Config struct {
	...
	CommiteeUpgrade *CommiteeUpgrade
...
func (cfg *Config) CommiteeFor(layer types.LayerID) uint16 {
	if cfg.CommiteeUpgrade != nil && layer >= cfg.CommiteeUpgrade.Layer {
		return cfg.CommiteeUpgrade.Size
	}
...
func (h *Hare) onLayer(layer types.LayerID) {
	...
	s := &session{
		...
		proto:   newProtocol(h.config.CommiteeFor(layer)/2 + 1),
	}
