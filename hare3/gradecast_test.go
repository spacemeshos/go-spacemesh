...
func TestGradecast(t *testing.T) {
	...
	assert.Equal(t, GradecastedSet{id1, values3, 2}, gc.RetrieveGradecastedMessages(r0)[0])

	// Grade 1, received 2 rounds after message round with max grade.
	gc = NewGradecaster()
	gc.ReceiveMsg(id1, values3, r0, r2, 3)
	assert.Equal(t, GradecastedSet{id1, values3, 1}, gc.RetrieveGradecastedMessages(r0)[0])
