...
func TestBuilder_HandleLayer_MultipleProposals(t *testing.T) {
	...
	tx1 := genTX(t, 1, generateAddress(t, []byte{0x01}), signing.NewEdSigner())
...
func TestBuilder_HandleLayer_OneProposal(t *testing.T) {
	...
	tx := genTX(t, 1, generateAddress(t, []byte{0x01}), signing.NewEdSigner())
...
func TestBuilder_UniqueBlockID(t *testing.T) {
	...
	require.NoError(t, layers.SetAggregatedHash(builder1.cdb, layerID.Sub(1), meshHash))
	b1, err := builder1.createProposal(context.TODO(), layerID, nil, atxID1, activeSet, beacon, nil, types.Votes{})
	require.NoError(t, err)

	require.NoError(t, layers.SetAggregatedHash(builder2.cdb, layerID.Sub(1), meshHash))
