...
func (t *BlockBuilder) createBlockLoop() {
	for {
		...
		case layerID := <-t.beginRoundEvent:
			...
			if err != nil {
				events.ReportDoneCreatingBlock(true, uint32(layerID), "failed to check for block eligibility")
				t.With().Error("failed to check for block eligibility", layerID, log.Err(err))
				continue
			}
			if len(proofs) == 0 {
				events.ReportDoneCreatingBlock(false, uint32(layerID), "")
			...
			for _, eligibilityProof := range proofs {
				...
				if err != nil {
					events.ReportDoneCreatingBlock(true, uint32(layerID), "failed to get txs for block")
					t.With().Error("failed to get txs for block", layerID, log.Err(err))
					continue
				}
				blk, err := t.createBlock(layerID, atxID, eligibilityProof, txList)
				if err != nil {
					events.ReportDoneCreatingBlock(true, uint32(layerID), "cannot create new block")
					t.Error("cannot create new block, %v ", err)
					continue
				}
				err = t.meshProvider.AddBlockWithTxs(blk, txs, nil)
				if err != nil {
					events.ReportDoneCreatingBlock(true, uint32(layerID), "failed to store block")
				...
				go func() {
					...
					if err != nil {
						...
						events.ReportDoneCreatingBlock(true, uint32(layerID), "cannot serialize block")
						return
					}
					...
					events.ReportDoneCreatingBlock(true, uint32(layerID), "")
				}()
			}
		}
	}
