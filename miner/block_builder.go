...
func (t *BlockBuilder) createBlockLoop() {
	for {
		...
		case layerID := <-t.beginRoundEvent:
			...
			if err != nil {
				events.Publish(events.DoneCreatingBlock{Eligible: true, Layer: uint32(layerID), Error: "failed to check for block eligibility"})
				t.With().Error("failed to check for block eligibility", layerID, log.Err(err))
				continue
			}
			if len(proofs) == 0 {
				events.Publish(events.DoneCreatingBlock{Eligible: false, Layer: uint32(layerID), Error: ""})
			...
			for _, eligibilityProof := range proofs {
				...
				if err != nil {
					events.Publish(events.DoneCreatingBlock{Eligible: true, Layer: uint32(layerID), Error: "failed to get txs for block"})
					t.With().Error("failed to get txs for block", layerID, log.Err(err))
					continue
				}
				blk, err := t.createBlock(layerID, atxID, eligibilityProof, txList)
				if err != nil {
					events.Publish(events.DoneCreatingBlock{Eligible: true, Layer: uint32(layerID), Error: "cannot create new block"})
					t.Error("cannot create new block, %v ", err)
					continue
				}
				err = t.meshProvider.AddBlockWithTxs(blk, txs, nil)
				if err != nil {
					events.Publish(events.DoneCreatingBlock{Eligible: true, Layer: uint32(layerID), Error: err.Error()})
				...
				go func() {
					...
					if err != nil {
						...
						events.Publish(events.DoneCreatingBlock{Eligible: true, Layer: uint32(layerID), Error: "cannot serialize block"})
						return
					}
					...
					events.Publish(events.DoneCreatingBlock{Eligible: true, Layer: uint32(layerID), Error: ""})
				}()
			}
		}
	}
