...
func TestNewAccountPendingTxs(t *testing.T) {
	...
	pendingTxs.Add([]types.TinyTx{
		types.AddressableTxToTiny(newTx(address.BytesToAddress(nil), 5, 100)),
	}, 1)
	empty = pendingTxs.IsEmpty()
	r.False(empty)
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	...
	pendingTxs.Remove([]types.TinyTx{
		types.AddressableTxToTiny(newTx(address.BytesToAddress(nil), 5, 50)),
	}, nil, 1)
	empty = pendingTxs.IsEmpty()
	r.True(empty)
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	...
	pendingTxs.Add([]types.TinyTx{
		types.AddressableTxToTiny(newTx(address.BytesToAddress(nil), 7, 100)),
		types.AddressableTxToTiny(newTx(address.BytesToAddress(nil), 8, 50)),
	}, 1)
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	r.Equal(prevNonce+1, nonce)
	r.Equal(prevBalance-100, balance)
	...
	pendingTxs.Add([]types.TinyTx{
		types.AddressableTxToTiny(newTx(address.BytesToAddress(nil), 6, 950)),
	}, 1)
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	r.Equal(prevNonce+1, nonce)
	r.Equal(prevBalance-100, balance)
	...
	pendingTxs.Add([]types.TinyTx{
		types.AddressableTxToTiny(newTx(address.BytesToAddress(nil), 6, 50)),
	}, 1)
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
