...
// DOING Validates block signature AND role VRF and that are from same public key
...
func setExpectations_ProducesExpectedBlockSignature_WhenChosenByOracle(
	...
	rig.rolacle.EXPECT().Proof(gomock.Any(), gomock.Any(), gomock.Any()).Return(signerRoleProof, nil)
...
func Test_GossipHandler_VerifiesSignatureAndRoleProof(t *testing.T) {
