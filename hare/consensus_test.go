...
func TestConsensusFixedOracle(t *testing.T) {
	...
	cfg := config.Config{N: 16, F: 8, RoundDuration: 2, ExpectedLeaders: 5, Limit: 1000}
...
func TestSingleValueForHonestSet(t *testing.T) {
	...
	cfg := config.Config{N: 50, F: 25, RoundDuration: 3, ExpectedLeaders: 5, Limit: 1000}
	totalNodes := 50
	sim := service.NewSimulator()
	test.initialSets = make([]*Set, totalNodes)
	set1 := NewSetFromValues(value1)
...
func TestAllDifferentSet(t *testing.T) {
	...
	cfg := config.Config{N: 10, F: 5, RoundDuration: 2, ExpectedLeaders: 5, Limit: 1000}
	sim := service.NewSimulator()
	totalNodes := 10
	test.initialSets = make([]*Set, totalNodes)
