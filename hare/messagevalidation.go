...
func (ev *eligibilityValidator) Validate(m *Msg) bool {
	...
	if err != nil {
		ev.With().Error("Error occurred while validating role", log.Err(err), log.NodeId(m.PubKey.ShortString()),
			log.LayerId(uint64(m.InnerMsg.InstanceId)), log.String("msg_type", m.InnerMsg.Type.String()))
		return false
	}
	// verify role
	if !res {
		ev.With().Warning("Validate message failed: role is invalid", log.NodeId(m.PubKey.ShortString()),
			log.LayerId(uint64(m.InnerMsg.InstanceId)), log.String("msg_type", m.InnerMsg.Type.String()))
		return false
	}
...
func (v *syntaxContextValidator) SyntacticallyValidateMessage(m *Msg) bool {
	...
	if m.InnerMsg.Values == nil {
		v.With().Warning("Syntax validation failed: set is nil",
			log.NodeId(m.PubKey.ShortString()), log.LayerId(uint64(m.InnerMsg.InstanceId)), log.String("msg_type", m.InnerMsg.Type.String()))
		return false
	}
...
func (v *syntaxContextValidator) validateSVP(msg *Msg) bool {
	...
	validateSameIteration := func(m *Msg) bool {
		...
		if proposalIter != statusIter { // not same iteration
			v.With().Warning("Proposal validation failed: not same iteration",
				log.NodeId(m.PubKey.ShortString()), log.LayerId(uint64(m.InnerMsg.InstanceId)),
	...
	if err := v.validateAggregatedMessage(msg.InnerMsg.Svp, validators); err != nil {
		v.With().Warning("Proposal validation failed: failed to validate aggregated messages", log.Err(err),
			log.NodeId(msg.PubKey.ShortString()), log.LayerId(uint64(msg.InnerMsg.InstanceId)))
		return false
	}
