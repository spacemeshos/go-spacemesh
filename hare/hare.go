...
type TerminationBlockOutput struct {
...
type Hare struct {
    ...
    blockIDSubscriptions []chan<- TerminationBlockOutput
...
// RegisterTerminationsCh adds a channel to a list of channels that will be
// provided with a TerminationBlockOutput upon termination of every hare process
func (h *Hare) RegisterTerminationsCh(sub chan<- TerminationBlockOutput) {
    h.blockIDSubscriptions = append(h.blockIDSubscriptions, sub)
}
func (h *Hare) updateTerminationSubscribers(layer types.LayerID,
    block types.BlockID, node types.NodeID) {

    terminationInfo := TerminationBlockOutput{
        LayerID:           layer,
        BlockID:           block,
        TerminatingNodeID: node,
    }
    for _, notifCh := range h.blockIDSubscriptions {
        notifCh <- terminationInfo
    }
}
...
func (h *Hare) collectOutput(ctx context.Context, output TerminationOutput) error {
    ...
    h.updateTerminationSubscribers(layerID, hareBlockIDOutput, h.nid)
