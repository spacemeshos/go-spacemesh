...
type Hare struct {
	...
	// channel to receive HareEligibilityGossip from the network via the MalfeasanceProof gossip handler.
	mchEligibility <-chan *types.HareEligibilityGossip
...
func New(
	...
	nid types.NodeID,
	ch chan LayerOutput,
	mch <-chan *types.HareEligibilityGossip,
...
func (h *Hare) malfeasanceLoop(ctx context.Context) {
	...
	for {
		select {
		case gossip := <-h.mchEligibility:
			h.broker.RelayEligibilityFromMalfeasanceGossip(ctx, gossip)
