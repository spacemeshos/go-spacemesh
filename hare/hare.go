...
type orphanBlockProvider interface {
	GetUnverifiedLayerBlocks(layerId block.LayerID) ([]block.BlockID, error)
}
...
type Hare struct {
	...
	lastLayer block.LayerID

	bufferSize int

	outputChan chan TerminationOutput
	mu         sync.RWMutex
	outputs    map[block.LayerID][]block.BlockID

	factory consensusFactory
}

// New returns a new Hare struct.
func New(conf config.Config, p2p NetworkService, sign Signing, obp orphanBlockProvider, rolacle Rolacle, beginLayer chan block.LayerID, logger log.Log) *Hare {
	h := new(Hare)
	...
	h.outputs = make(map[block.LayerID][]block.BlockID, h.bufferSize) //  we keep results about LayerBuffer past layers
...
func (h *Hare) collectOutput(output TerminationOutput) error {
	...
	set := output.Set()
	blocks := make([]block.BlockID, len(set.values))
	i := 0
	...
	h.outputs[block.LayerID(id)] = blocks
	h.mu.Unlock()

	return nil
}

func (h *Hare) onTick(id block.LayerID) {
	h.layerLock.Lock()
	if id > h.lastLayer {
		h.lastLayer = id
	}
...
func (h *Hare) BlockingGetResult(id block.LayerID) ([]block.BlockID, error) {
...
// GetResults returns the hare output for a given LayerID. returns error if we don't have results yet.
func (h *Hare) GetResult(id block.LayerID) ([]block.BlockID, error) {
	if h.isTooLate(InstanceId(id)) {
		return nil, ErrTooOld
	}
