...
func Test_16Nodes_HareIntegrationSuite(t *testing.T) {
	...
	his.BeforeHook = func(idx int, s p2p.NodeTestInstance) {
		...
		broker := newBroker(s, newEligibilityValidator(eligibility.New(), 10, &mockIdProvider{}, cfg.N, cfg.ExpectedLeaders, lg), NewMockStateQuerier(), (&mockSyncer{true}).IsSynced, 10, cfg.Limit, Closer{}, lg)
