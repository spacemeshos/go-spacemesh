...
// GetAtxIDsByCoinbaseAndLayer returns a list of all atx ids corresponding to a coinbase address after start layer
func (msh *Mesh) GetAtxIDsByCoinbaseAndLayer(coinbase types.Address, startLayer types.LayerID) (atxIds []types.ATXID, err error) {
	...
	for atxIter.Next() {
		var a types.ATXID
		if err := types.BytesToInterface(atxIter.Value(), &a); err != nil {
			return nil, err
		}
