...
func TestMesh_ProcessLayerPerHareOutput(t *testing.T) {
	...
	for i := gPlus1; !i.After(gPlus5); i = i.Add(1) {
		...
		tm.mockState.EXPECT().GetStateRoot().Return(types.Hash32{}, nil)
		require.NoError(t, tm.ProcessLayerPerHareOutput(context.TODO(), i, toApply.ID(), false))
...
func TestMesh_ProcessLayerPerHareOutput_OutOfOrder(t *testing.T) {
	...
	require.NoError(t, tm.ProcessLayerPerHareOutput(context.TODO(), gPlus1, blocks1[0].ID(), false))
	...
	err = tm.ProcessLayerPerHareOutput(context.TODO(), gPlus3, blocks3[0].ID(), false)
	...
	err = tm.ProcessLayerPerHareOutput(context.TODO(), gPlus5, blocks5[0].ID(), false)
	require.ErrorIs(t, err, errMissingHareOutput)
	...
	err = tm.ProcessLayerPerHareOutput(context.TODO(), gPlus2, blocks2[0].ID(), false)
	require.ErrorIs(t, err, errMissingHareOutput)
	...
	require.NoError(t, tm.ProcessLayerPerHareOutput(context.TODO(), gPlus4, blocks4[0].ID(), false))
...
func TestMesh_ProcessLayerPerHareOutput_emptyOutput(t *testing.T) {
	...
	tm.mockState.EXPECT().GetStateRoot().Return(types.Hash32{}, nil)
	require.NoError(t, tm.ProcessLayerPerHareOutput(context.TODO(), gPlus2, types.EmptyBlockID, false))
...
func TestMesh_Revert(t *testing.T) {
	...
	for i := gPlus1; i.Before(gPlus4); i = i.Add(1) {
		...
		tm.mockState.EXPECT().GetStateRoot().Return(types.Hash32{}, nil)
		require.NoError(t, tm.ProcessLayerPerHareOutput(context.TODO(), i, applied.ID(), false))
	}
	require.Equal(t, gPlus3, tm.ProcessedLayer())
	require.Equal(t, gPlus3, tm.LatestLayerInState())
	checkLastAppliedInDB(t, tm.Mesh, gPlus3)
	...
	tm.mockState.EXPECT().ApplyLayer(context.TODO(), gPlus5, blocks5[0]).Return(nil)
	tm.mockState.EXPECT().GetStateRoot().Return(types.Hash32{}, nil)
	require.NoError(t, tm.ProcessLayerPerHareOutput(context.TODO(), gPlus5, blocks5[0].ID(), false))
	require.Equal(t, gPlus5, tm.ProcessedLayer())
	require.Equal(t, gPlus5, tm.LatestLayerInState())
	checkLastAppliedInDB(t, tm.Mesh, gPlus5)
