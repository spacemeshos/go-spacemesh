...
func TestFixedRolacle_Eligible(t *testing.T) {
	...
	res, err := oracle.eligible(context.Background(), types.LayerID(1), 1, 10, v)
	require.NoError(t, err)
	res2, err := oracle.eligible(context.Background(), types.LayerID(1), 1, 10, v)
	require.NoError(t, err)
...
func TestFixedRolacle_Eligible2(t *testing.T) {
	...
	count := 0
	for _, p := range pubs {
		res, _ := oracle.eligible(context.Background(), types.LayerID(1), 1, 10, p)
		if res {
			count++
		}
	}
	...
	count = 0
	for _, p := range pubs {
		res, _ := oracle.eligible(context.Background(), types.LayerID(1), 1, 20, p)
		if res {
			count++
		}
	}
...
func TestFixedRolacle_Range(t *testing.T) {
	...
	count := 0
	for _, p := range pubs {
		res, _ := oracle.eligible(context.Background(), types.LayerID(1), 1, numOfClients, p)
		if res {
			count++
		}
	}
	...
	count = 0
	for _, p := range pubs {
		res, _ := oracle.eligible(context.Background(), types.LayerID(2), 1, 0, p)
		if res {
			count++
		}
	}
...
func TestFixedRolacle_Eligible3(t *testing.T) {
	...
	exp := numOfClients / 2
	ok, err := oracle.eligible(context.Background(), types.LayerID(1), 1, exp, types.NodeID{1})
	require.NoError(t, err)
	require.False(t, ok)

	hc := 0
	for k := range oracle.honest {
		res, _ := oracle.eligible(context.Background(), types.LayerID(1), 1, exp, k)
		if res {
			hc++
		}
	}
...
func TestFixedRolacle_Eligible4(t *testing.T) {
	...
	for _, s := range ids {
		res, _ := oracle.eligible(context.Background(), 0, 1, numOfClients, s)
		assert.True(t, res)
	}
}
