...
func SyncMockFactory(number int, conf Configuration, name string, dbType string) (syncs []*Syncer, p2ps []*service.Node) {
	...
	for i := 0; i < number; i++ {
		...
		l := log.New(name, "", "")
		sync := NewSync(net, getMesh(dbType, Path+name+"_"+time.Now().String()), BlockValidatorMock{}, TxValidatorMock{}, conf, tk, 0, l)
		ts.Start()
...
func Test_TwoNodes_SyncIntegrationSuite(t *testing.T) {
	...
	sis.BeforeHook = func(idx int, s p2p.NodeTestInstance) {
		...
		sync := NewSync(s, msh, BlockValidatorMock{}, TxValidatorMock{}, conf, tk, 0, l)
