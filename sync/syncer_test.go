...
func SyncMockFactory(number int, conf Configuration, name string, dbType string, poetDb func() PoetDb) (syncs []*Syncer, p2ps []*service.Node) {
	...
	for i := 0; i < number; i++ {
		...
		sync := NewSync(net, getMesh(dbType, Path+name+"_"+time.Now().String()), txpool, atxpool, mockTxProcessor{}, blockValidator, poetDb(), conf, ts, 0, l)
		ts.StartNotifying()
	...
	for {
		...
		default:
			if syncObj3.ValidatedLayer() == 5 {
				...
				fmt.Println("safdsaf  ", syncObj3.ValidatedLayer())
			}
