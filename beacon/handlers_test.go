...
func createProtocolDriverWithFirstRoundVotes(
	...
	tpd.setBeginProtocol(tb.Context())
...
func Test_HandleProposal_InitEpoch(t *testing.T) {
	...
	tpd.setBeginProtocol(t.Context())
	...
	res := tpd.HandleProposal(t.Context(), "peerID", msgBytes)
...
func Test_HandleProposal_Success(t *testing.T) {
	...
	minerAtxs := map[types.NodeID]*minerInfo{
		signer1.NodeID(): {
			atxid: createATX(t, tpd.cdb, epoch.FirstLayer().Sub(1), signer1, 10, epochStart.Add(-1*time.Minute)),
		},
		signer2.NodeID(): {
			atxid: createATX(t, tpd.cdb, epoch.FirstLayer().Sub(1), signer2, 10, epochStart.Add(-2*time.Minute)),
		},
	}
	...
	require.NoError(t, tpd.HandleProposal(t.Context(), "peerID", msgBytes1))
	...
	require.NoError(t, tpd.HandleProposal(t.Context(), "peerID", msgBytes2))
...
func Test_HandleProposal_Malicious(t *testing.T) {
	t.Parallel()
	...
	minerAtxs := map[types.NodeID]*minerInfo{
		signer1.NodeID(): {
			atxid:     createATX(t, tpd.cdb, epoch.FirstLayer().Sub(1), signer1, 10, epochStart.Add(-2*time.Minute)),
			malicious: true,
		},
		signer2.NodeID(): {
			atxid: createATX(t, tpd.cdb, epoch.FirstLayer().Sub(1), signer2, 10, epochStart.Add(-1*time.Minute)),
		},
	}
	st := createEpochState(t, tpd.ProtocolDriver, epoch, minerAtxs, mockChecker)
	tpd.mClock.EXPECT().LayerToTime(epoch.FirstLayer()).Return(epochStart).AnyTimes()
...
func Test_HandleProposal_NotInProtocolStillWorks(t *testing.T) {
	...
	minerAtxs := map[types.NodeID]*minerInfo{
		signer.NodeID(): {
			atxid: createATX(t, tpd.cdb, epoch.FirstLayer().Sub(1), signer, 10, epochStart.Add(-1*time.Minute)),
		},
	}

	createEpochState(t, tpd.ProtocolDriver, epoch, minerAtxs, mockChecker)

	tpd.setEndProtocol(t.Context())
...
func Test_handleProposal_Corrupted(t *testing.T) {
	...
	got := tpd.HandleProposal(t.Context(), "peerID", msg)
...
func Test_handleProposal_EpochTooOld(t *testing.T) {
	...
	got := tpd.HandleProposal(t.Context(), "peerID", msgBytes)
...
func Test_handleProposal_NextEpoch(t *testing.T) {
	...
	mockChecker := NewMockeligibilityChecker(gomock.NewController(t))
	minerAtxs := map[types.NodeID]*minerInfo{
		signer.NodeID(): {atxid: createATX(t, tpd.cdb, epoch.FirstLayer().Sub(1), signer, 10, now)},
	}
...
func Test_handleProposal_AlreadyProposed(t *testing.T) {
	...
	tpd.mClock.EXPECT().LayerToTime(epoch.FirstLayer()).Return(epochStart).AnyTimes()
	mockChecker.EXPECT().PassStrictThreshold(gomock.Any()).Return(true)
	got := tpd.HandleProposal(t.Context(), "peerID", msgBytes1)
	require.NoError(t, got)
	...
	tpd.mClock.EXPECT().CurrentLayer().Return(epoch.FirstLayer())
	got = tpd.HandleProposal(t.Context(), "peerID", msgBytes2)
	...
	minerAtxs := map[types.NodeID]*minerInfo{
		signer.NodeID(): {
			atxid: createATX(
				t,
				tpd.cdb,
				epoch.FirstLayer().Sub(1),
				signer,
				10,
				epochStart.Add(tpd.config.GracePeriodDuration),
			),
		},
	}
	createEpochState(t, tpd.ProtocolDriver, epoch, minerAtxs, mockChecker)
	...
	minerAtxs := map[types.NodeID]*minerInfo{
		signer.NodeID(): {
			atxid: createATX(
				...
				signer,
				10,
				epochStart.Add(2*tpd.config.GracePeriodDuration),
			),
		},
	}
	createEpochState(t, tpd.ProtocolDriver, epoch, minerAtxs, mockChecker)
...
func Test_HandleFirstVotes_Success(t *testing.T) {
	...
	tpd.mClock.EXPECT().CurrentLayer().Return(epoch.FirstLayer())
	tpd.mClock.EXPECT().LayerToTime(gomock.Any()).Return(time.Now()).AnyTimes()
	res := tpd.HandleFirstVotes(t.Context(), "peerID", msgBytes)
	require.NoError(t, res)
...
func Test_HandleFirstVotes_Malicious(t *testing.T) {
	t.Parallel()
	...
	minerAtxs := map[types.NodeID]*minerInfo{
		signer.NodeID(): {
			atxid:     createATX(t, tpd.cdb, epoch.FirstLayer().Sub(1), signer, 10, time.Now()),
			malicious: true,
		},
	}
	createEpochState(t, tpd.ProtocolDriver, epoch, minerAtxs, nil)
...
func Test_handleFirstVotes_CorruptMsg(t *testing.T) {
	...
	got := tpd.HandleFirstVotes(t.Context(), "peerID", msgBytes[1:])
	require.ErrorIs(t, got, errMalformedMessage)
	checkVoted(t, tpd.ProtocolDriver, epoch, signer, types.FirstRound, false)
	checkFirstIncomingVotes(t, tpd.ProtocolDriver, epoch, map[types.NodeID]proposalList{})
}
...
func Test_handleFirstVotes_WrongEpoch(t *testing.T) {
	...
	tpd.mClock.EXPECT().LayerToTime(gomock.Any()).Return(time.Now()).AnyTimes()
	tpd.mClock.EXPECT().CurrentLayer().Return(epoch.FirstLayer())
	got := tpd.HandleFirstVotes(t.Context(), "peerID", msgBytes)
...
func Test_HandleFirstVotes_AlreadyVoted(t *testing.T) {
	...
	tpd.mClock.EXPECT().CurrentLayer().Return(epoch.FirstLayer())
	got = tpd.HandleFirstVotes(t.Context(), "peerID", msgBytes2)
...
func Test_HandleFollowingVotes_Success(t *testing.T) {
	...
	tpd.mClock.EXPECT().CurrentLayer().Return(epoch.FirstLayer())
	tpd.mClock.EXPECT().LayerToTime(gomock.Any()).Return(time.Now()).AnyTimes()
	res := tpd.HandleFollowingVotes(t.Context(), "peerID", msgBytes)
	require.NoError(t, res)
...
func Test_HandleFollowingVotes_Malicious(t *testing.T) {
	t.Parallel()
...
func Test_handleFollowingVotes_CorruptMsg(t *testing.T) {
	...
	got := tpd.HandleFollowingVotes(t.Context(), "peerID", msgBytes[1:])
	require.ErrorIs(t, got, errMalformedMessage)
	checkVoted(t, tpd.ProtocolDriver, epoch, signer, round, false)
	checkVoteMargins(t, tpd.ProtocolDriver, epoch, emptyVoteMargins(plist))
}
...
func Test_handleFollowingVotes_WrongEpoch(t *testing.T) {
	...
	tpd.mClock.EXPECT().CurrentLayer().Return(epoch.FirstLayer())
	got := tpd.HandleFollowingVotes(t.Context(), "peerID", msgBytes)
	require.ErrorIs(t, got, errEpochNotActive)
	checkVoted(t, tpd.ProtocolDriver, epoch, signer, round, false)
	checkVoteMargins(t, tpd.ProtocolDriver, epoch, emptyVoteMargins(plist))
