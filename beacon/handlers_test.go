...
func setMinerFirstRoundVotes(t *testing.T, pd *ProtocolDriver, epoch types.EpochID, nodeID types.NodeID, minerFirstRound proposalList) {
	t.Helper()
	pd.mu.Lock()
	defer pd.mu.Unlock()
	pd.states[epoch].setMinerFirstRoundVote(nodeID, minerFirstRound)
}
