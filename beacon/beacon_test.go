...
func TestBeaconNotSynced_ReleaseMemory(t *testing.T) {
	...
	for eid := start; eid <= end; eid++ {
		b := types.NewExistingBallot(types.RandomBallotID(), [64]byte{}, types.EmptyNodeID, types.BallotMetadata{
...
func TestBeacon_ReportBeaconFromBallot_SameBallot(t *testing.T) {
	...
	b1 := types.NewExistingBallot(types.RandomBallotID(), [64]byte{}, types.EmptyNodeID, types.BallotMetadata{
		Layer: epoch.FirstLayer(),
	})
	...
	b2 := types.NewExistingBallot(types.RandomBallotID(), [64]byte{}, types.EmptyNodeID, types.BallotMetadata{
		Layer: epoch.FirstLayer(),
	})
