...
func TestCachedDb_GetEpochWeight(t *testing.T) {
	t.Run("empty epoch", func(t *testing.T) {
		db := sql.InMemory()
		cdb := datastore.NewCachedDB(db, logtest.New(t))

		weight, err := cdb.GetEpochWeight(1)
		require.NoError(t, err)
		require.Equal(t, uint64(0), weight)
	})
	t.Run("epoch with weight", func(t *testing.T) {
		db := sql.InMemory()
		cdb := datastore.NewCachedDB(db, logtest.New(t))
		epoch := types.EpochID(2)
		total := uint32(0)
		// put some ATXs in
		for publishEpoch := range epoch + 1 {
			for i := range uint32(10) {
				...
				if atx.PublishEpoch == epoch {
					total += atx.NumUnits
				}
			}
		}

		got, err := cdb.GetEpochWeight(epoch + 1)
		require.NoError(t, err)
		require.Equal(t, uint64(total), got)
	})
}
