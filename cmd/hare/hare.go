...
func (app *HareApp) Start(cmd *cobra.Command, args []string) {
	...
	app.ha = hare.New(app.Config.HARE, app.p2p, app.sgn, types.NodeId{Key: app.sgn.PublicKey().String(), VRFPublicKey: []byte{}}, IsSynced, &mockBlockProvider{}, hareOracle, app.Config.CONSENSUS.LayersPerEpoch, &mockIdProvider{}, &mockStateQuerier{}, app.clock.Subscribe(), func() bool { return true }, lg)
