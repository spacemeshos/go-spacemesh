...
func AddFlags(flagSet *pflag.FlagSet, cfg *config.Config) (configPath *string) {
	...
	/** ======================== Checkpoint Flags ========================== **/
	flagSet.StringVar(&cfg.Recovery.Uri, "recovery-uri",
		cfg.Recovery.Uri, "reset the node state based on the supplied checkpoint file",
	)
	flagSet.Uint32Var(&cfg.Recovery.Restore, "recovery-layer",
		cfg.Recovery.Restore, "restart the mesh with the checkpoint file at this layer",
	)
	...
	flagSet.StringVar(&cfg.BaseConfig.FileLock, "filelock",
		cfg.BaseConfig.FileLock, "Filesystem lock to prevent running more than one instance.",
	)
	flagSet.StringVar(&cfg.LOGGING.Encoder, "log-encoder", cfg.LOGGING.Encoder, "Log as JSON instead of plain text")
	flagSet.BoolVar(&cfg.CollectMetrics, "metrics", cfg.CollectMetrics, "collect node metrics")
	flagSet.IntVar(&cfg.MetricsPort, "metrics-port", cfg.MetricsPort, "metric server port")
	...
	flagSet.Var(&flags.JSONFlag{Value: &cfg.PoetServers}, "poet-servers",
	...
	flagSet.DurationVar(&cfg.LayerDuration, "layer-duration", cfg.LayerDuration, "Duration between layers")
	flagSet.Uint32Var(&cfg.LayerAvgSize, "layer-average-size", cfg.LayerAvgSize, "Layer Avg size")
	flagSet.BoolVar(&cfg.PprofHTTPServer, "pprof-server", cfg.PprofHTTPServer, "enable http pprof server")
	...
	flagSet.StringVar(&cfg.PprofHTTPServerListener, "pprof-listener",
		cfg.PprofHTTPServerListener, "Listen address for pprof server, not safe to expose publicly",
	)
	...
	flagSet.StringVar(&cfg.ProfilerName, "profiler-name", cfg.ProfilerName, "the name to use when sending profiles")

	flagSet.IntVar(&cfg.TxsPerProposal, "txs-per-proposal",
		cfg.TxsPerProposal, "the number of transactions to select per proposal",
	)
	flagSet.Uint64Var(&cfg.BlockGasLimit, "block-gas-limit", cfg.BlockGasLimit, "max gas allowed per block")
	...
	flagSet.Var(flags.NewAddressListValue(cfg.P2P.Listen, &cfg.P2P.Listen), "listen", "address(es) for listening")
	flagSet.BoolVar(&cfg.P2P.Flood, "flood", cfg.P2P.Flood, "flood created messages to all peers")
	...
	flagSet.BoolVar(&cfg.P2P.DisableReusePort, "disable-reuseport", cfg.P2P.DisableReusePort,
		"disables SO_REUSEPORT for tcp sockets. Try disabling this if your node can't reach bootnodes in the network",
	)
	flagSet.BoolVar(&cfg.P2P.Metrics, "p2p-metrics",
		cfg.P2P.Metrics, "enable extended metrics collection from libp2p components",
	)
	flagSet.IntVar(&cfg.P2P.AcceptQueue, "p2p-accept-queue",
		cfg.P2P.AcceptQueue, "number of connections that are fully setup before accepting new connections",
	)
	flagSet.IntVar(&cfg.P2P.LowPeers, "low-peers", cfg.P2P.LowPeers, "low watermark for the number of connections")
	flagSet.IntVar(&cfg.P2P.HighPeers, "high-peers", cfg.P2P.HighPeers,
	...
	flagSet.StringSliceVar(&cfg.P2P.Bootnodes, "bootnodes", cfg.P2P.Bootnodes, "entrypoints into the network")
	...
	flagSet.StringSliceVar(&cfg.P2P.StaticRelays, "static-relays", cfg.P2P.StaticRelays, "static relay list")
	flagSet.Var(flags.NewAddressListValue(cfg.P2P.AdvertiseAddress, &cfg.P2P.AdvertiseAddress),
		"advertise-address", "libp2p address(es) with identity (example: /dns4/bootnode.spacemesh.io/tcp/5003)",
	)
	...
	flagSet.BoolVar(&cfg.P2P.ForceDHTServer, "force-dht-server", cfg.P2P.ForceDHTServer, "force DHT server mode")
	flagSet.BoolVar(&cfg.P2P.EnableTCPTransport, "enable-tcp-transport",
		cfg.P2P.EnableTCPTransport, "enable TCP transport",
	)
	flagSet.BoolVar(&cfg.P2P.EnableQUICTransport, "enable-quic-transport",
		cfg.P2P.EnableQUICTransport, "enable QUIC transport",
	)
	flagSet.BoolVar(&cfg.P2P.EnableRoutingDiscovery, "enable-routing-discovery",
		cfg.P2P.EnableQUICTransport, "enable routing discovery",
	)
	...
	flagSet.Uint64Var(&cfg.POST.LabelsPerUnit, "post-labels-per-unit", cfg.POST.LabelsPerUnit, "")
	flagSet.Uint32Var(&cfg.POST.MinNumUnits, "post-min-numunits", cfg.POST.MinNumUnits, "")
	flagSet.Uint32Var(&cfg.POST.MaxNumUnits, "post-max-numunits", cfg.POST.MaxNumUnits, "")
	flagSet.UintVar(&cfg.POST.K1, "post-k1", cfg.POST.K1,
		"difficulty factor for finding a good label when generating a proof",
	)
	flagSet.UintVar(&cfg.POST.K2, "post-k2", cfg.POST.K2, "number of labels to prove")
	flagSet.UintVar(&cfg.POST.K3, "post-k3", cfg.POST.K3,
	...
	flagSet.StringVar(&cfg.SMESHING.Opts.DataDir, "smeshing-opts-datadir", cfg.SMESHING.Opts.DataDir, "")
	flagSet.Uint32Var(&cfg.SMESHING.Opts.NumUnits, "smeshing-opts-numunits", cfg.SMESHING.Opts.NumUnits, "")
	flagSet.Uint64Var(&cfg.SMESHING.Opts.MaxFileSize, "smeshing-opts-maxfilesize", cfg.SMESHING.Opts.MaxFileSize, "")
	flagSet.AddFlag(&pflag.Flag{
		Name:     "smeshing-opts-provider",
		Value:    &cfg.SMESHING.Opts.ProviderID,
		DefValue: cfg.SMESHING.Opts.ProviderID.String(),
	})
	flagSet.BoolVar(&cfg.SMESHING.Opts.Throttle, "smeshing-opts-throttle", cfg.SMESHING.Opts.Throttle, "")
	...
	flagSet.BoolVar(&cfg.SMESHING.VerifyingOpts.Disabled, "smeshing-opts-verifying-disable", false,
	...
	flagSet.IntVar(&cfg.SMESHING.VerifyingOpts.MinWorkers, "smeshing-opts-verifying-min-workers",
	...
	flagSet.Uint32Var(&cfg.LayersPerEpoch, "layers-per-epoch", cfg.LayersPerEpoch, "number of layers in epoch")
	...
	flagSet.StringVar(&cfg.Bootstrap.URL, "bootstrap-url", cfg.Bootstrap.URL, "the url to query bootstrap data update")
