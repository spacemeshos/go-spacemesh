...
func (app *SyncApp) Start(cmd *cobra.Command, args []string) {
	...
	atxdb := activation.NewActivationDb(database.NewMemDatabase(), activation.NewIdentityStore(iddbstore), database.NewMemDatabase(), uint64(app.Config.CONSENSUS.LayersPerEpoch), validator, lg.WithName("atxDb").WithOptions(log.Nop))
	mshDb := mesh.NewPersistentMeshDB(app.Config.DataDir, atxdb, lg.WithOptions(log.Nop))
