BOOT_DEPLOYMENT_FILE = './k8s/bootstrapoet-w-conf.yml'
BOOT_STATEFULSET_FILE = './k8s/bootstrapoet-w-conf-ss.yml'
CLIENT_DEPLOYMENT_FILE = './k8s/client-w-conf.yml'
CLIENT_STATEFULSET_FILE = './k8s/client-w-conf-ss.yml'
CURL_POD_FILE = './k8s/curl.yml'
ELASTIC_CONF_DIR = './tests/elk/elastic/'
# FILEBEAT_CONF_DIR = './tests/elk/filebeat_test/'
FILEBEAT_CONF_DIR = './tests/elk/filebeat/'
FLUENTBIT_CONF_DIR = './tests/elk/fluent-bit/'
KIBANA_CONF_DIR = './tests/elk/kibana/'
LOGSTASH_CONF_DIR = './tests/elk/logstash/'

BOOTSTRAP_PORT = 7513
ORACLE_SERVER_PORT = 3030
POET_SERVER_PORT = 80
