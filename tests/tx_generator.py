...
class WalletAPI:
    ...
    def SubmitTx(self, to , src, gaslimit, gasprice,amount, txgen):
        ...
        out = api_call(pod_ip, data, self.submitapi, testconfig['namespace'])
        if DEBUG:
            print(data)
            print(out)
    ...
    def CheckNonce(self, acc):
        ...
        return api_call(pod_ip, data, nonceapi, testconfig['namespace'])
...
layers_per_epoch = testconfig['client']['args']['layers-per-epoch']
layers_duration = testconfig['client']['args']['layer-duration-sec']
