...
from .transaction import Address, TxType, IncompleteTransaction, TransactionBody, \
    TX_SPAWN_APP, TX_SIGNING_ED, TX_SIGNING_ED_PLUS, \
...
class SpawnAppTx(TransactionBody):
    ttl: int
    nonce: int
    app_address: Address
    amount: int
    ...
    def new_ed(self):
        return IncompleteTransaction(SPAWN_APP_ED_TX, self)

    def new_ed_plus(self):
        return IncompleteTransaction(SPAWN_APP_ED_PLUS_TX, self)
    ...
    def __str__(self) -> str:
        return "SpawnAppTx(ttl=%d, nonce=%d, app_address=%s, amount=%d, gas_limit=%d, gas_price=%d)" % \
               (self.ttl, self.nonce, str(self.app_address), self.amount, self.gas_limit, self.gas_price)


def decode_spawn_app_tx(bs: bytes, tt: TxType) -> IncompleteTransaction:
    ...
    p.done()
    return IncompleteTransaction(tt, SpawnAppTx(ttl, nonce, Address(addr), amount, gas_limit, gas_price, call_data))


SPAWN_APP_ED_TX = TxType(
    TX_SPAWN_APP + TX_SIGNING_ED, "SPAWN_APP_ED_TX",
    EdSigningScheme,
    decode_spawn_app_tx)

SPAWN_APP_ED_PLUS_TX = TxType(
    TX_SPAWN_APP + TX_SIGNING_ED_PLUS, "SPAWN_APP_ED_PLUS_TX",
    EdPlusSignigScheme,
    decode_spawn_app_tx)
