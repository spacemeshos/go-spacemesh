...
def test_atxs_nodes_up(setup_bootstrap, setup_clients, add_curl, wait_genesis, setup_poet, setup_oracle):
    ...
    added_clients = add_multi_clients(setup_bootstrap.deployment_id,
                                      get_conf(setup_bootstrap.pods[0], setup_poet, setup_oracle), 20)

    queries.wait_for_latest_layer(testconfig["namespace"], last_layer)
    analyse.analyze_mining(testconfig['namespace'], last_layer, layers_per_epoch, layer_avg_size, layer_avg_size + extra_nodes)
