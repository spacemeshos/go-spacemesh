...
func (s NodeService) Status(ctx context.Context, request *pb.StatusRequest) (*pb.StatusResponse, error) {
	...
	return &pb.StatusResponse{
		Status: &pb.NodeStatus{
			...
			SyncedLayer:    s.Tx.LatestLayer().Uint32(),          // latest layer we saw from the network
			TopLayer:       s.GenTime.GetCurrentLayer().Uint32(), // current layer, based on time
			VerifiedLayer:  s.Tx.LatestLayerInState().Uint32(),   // latest verified layer
		},
	}, nil
}
