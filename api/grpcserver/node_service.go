...
func (s NodeService) Status(ctx context.Context, request *pb.StatusRequest) (*pb.StatusResponse, error) {
	...
	return &pb.StatusResponse{
		Status: &pb.NodeStatus{
			...
			SyncedLayer:    uint64(s.Mesh.LatestLayer().Uint32()),        // latest layer we saw from the network
			TopLayer:       uint64(s.GenTime.GetCurrentLayer().Uint32()), // current layer, based on time
			VerifiedLayer:  uint64(s.Mesh.LatestLayerInState().Uint32()), // latest verified layer
		},
	}, nil
}
