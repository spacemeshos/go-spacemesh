...
func TestLayerService_List(t *testing.T) {
	setup := func(t *testing.T) spacemeshv2beta1.LayerServiceClient {
		...
		conn := dialGrpc(t, cfg)
		return spacemeshv2beta1.NewLayerServiceClient(conn)
	}

	t.Run("limit set too high", func(t *testing.T) {
		client := setup(t)
		_, err := client.List(context.Background(), &spacemeshv2beta1.LayerRequest{Limit: 200})
		require.Error(t, err)
	...
	t.Run("no limit set", func(t *testing.T) {
		client := setup(t)
		_, err := client.List(context.Background(), &spacemeshv2beta1.LayerRequest{})
		require.Error(t, err)
	...
	t.Run("limit and offset", func(t *testing.T) {
		client := setup(t)
		list, err := client.List(context.Background(), &spacemeshv2beta1.LayerRequest{
			Limit:  25,
			Offset: 50,
		})
		require.NoError(t, err)
	...
	t.Run("all", func(t *testing.T) {
		client := setup(t)
		ls, err := client.List(context.Background(), &spacemeshv2beta1.LayerRequest{
			StartLayer: 0,
			EndLayer:   100,
			Limit:      100,
		})
...
func TestLayerConvertEventStatus(t *testing.T) {
	...
	require.Equal(t, spacemeshv2beta1.Layer_LAYER_STATUS_APPLIED, s)

	s = convertEventStatus(events.LayerStatusTypeConfirmed)
	require.Equal(t, spacemeshv2beta1.Layer_LAYER_STATUS_VERIFIED, s)

	s = convertEventStatus(events.LayerStatusTypeApplied)
	require.Equal(t, spacemeshv2beta1.Layer_LAYER_STATUS_VERIFIED, s)

	s = convertEventStatus(events.LayerStatusTypeUnknown)
	require.Equal(t, spacemeshv2beta1.Layer_LAYER_STATUS_UNSPECIFIED, s)
}

func TestLayerStreamService_Stream(t *testing.T) {
	setup := func(t *testing.T, db sql.StateDatabase) (spacemeshv2beta1.LayerStreamServiceClient, []layers.Layer) {
		...
		conn := dialGrpc(t, cfg)
		return spacemeshv2beta1.NewLayerStreamServiceClient(conn), lrs
	}
	...
	t.Run("all", func(t *testing.T) {
		...
		stream, err := client.Stream(context.Background(), &spacemeshv2beta1.LayerStreamRequest{})
		require.NoError(t, err)
