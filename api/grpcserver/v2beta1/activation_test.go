...
func TestActivationService_List(t *testing.T) {
	setup := func(t *testing.T) (spacemeshv2beta1.ActivationServiceClient, []types.ActivationTx) {
		...
		conn := dialGrpc(t, cfg)
		return spacemeshv2beta1.NewActivationServiceClient(conn), activations
	}

	t.Run("limit set too high", func(t *testing.T) {
		client, _ := setup(t)
		_, err := client.List(context.Background(), &spacemeshv2beta1.ActivationRequest{Limit: 200})
		require.Error(t, err)
	...
	t.Run("no limit set", func(t *testing.T) {
		client, _ := setup(t)
		_, err := client.List(context.Background(), &spacemeshv2beta1.ActivationRequest{})
		require.Error(t, err)
	...
	t.Run("limit and offset", func(t *testing.T) {
		client, _ := setup(t)
		list, err := client.List(context.Background(), &spacemeshv2beta1.ActivationRequest{
			Limit:  25,
			Offset: 50,
		})
	...
	t.Run("all", func(t *testing.T) {
		...
		list, err := client.List(context.Background(), &spacemeshv2beta1.ActivationRequest{Limit: 100})
		require.NoError(t, err)
...
func TestActivationStreamService_Stream(t *testing.T) {
	setup := func(t *testing.T, db sql.Executor) spacemeshv2beta1.ActivationStreamServiceClient {
		gen := fixture.NewAtxsGenerator()
		activations := make([]types.ActivationTx, 100)
		for i := range activations {
			atx := gen.Next()
		...
		conn := dialGrpc(t, cfg)
		return spacemeshv2beta1.NewActivationStreamServiceClient(conn)
	}
	...
	t.Run("all", func(t *testing.T) {
		...
		stream, err := client.Stream(context.Background(), &spacemeshv2beta1.ActivationStreamRequest{})
		require.NoError(t, err)
...
func TestActivationService_ActivationsCount(t *testing.T) {
	...
	t.Run("count without filter", func(t *testing.T) {
		count, err := client.ActivationsCount(ctx, &spacemeshv2beta1.ActivationsCountRequest{})
		require.NoError(t, err)
	...
	t.Run("count with filter", func(t *testing.T) {
		epoch := uint32(3)
		epoch3Count, err := client.ActivationsCount(ctx, &spacemeshv2beta1.ActivationsCountRequest{
			Epoch: &epoch,
		})
		require.NoError(t, err)
		...
		epoch = uint32(5)
		epoch5Count, err := client.ActivationsCount(ctx, &spacemeshv2beta1.ActivationsCountRequest{
			Epoch: &epoch,
		})
		require.NoError(t, err)
