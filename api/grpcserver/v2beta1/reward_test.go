...
func TestRewardService_List(t *testing.T) {
	setup := func(t *testing.T) (spacemeshv2beta1.RewardServiceClient, []types.Reward) {
		...
		conn := dialGrpc(t, cfg)
		return spacemeshv2beta1.NewRewardServiceClient(conn), rwds
	}
	...
	t.Run("coinbase", func(t *testing.T) {
		...
		list, err := client.List(t.Context(), &spacemeshv2beta1.RewardRequest{
			...
			FilterBy:   &spacemeshv2beta1.RewardRequest_Coinbase{Coinbase: rwds[3].Coinbase.String()},
		})
		require.NoError(t, err)
	...
	t.Run("smesher", func(t *testing.T) {
		...
		list, err := client.List(t.Context(), &spacemeshv2beta1.RewardRequest{
			...
			FilterBy:   &spacemeshv2beta1.RewardRequest_Smesher{Smesher: rwds[4].SmesherID.Bytes()},
		})
		require.NoError(t, err)
...
func TestRewardStreamService_Stream(t *testing.T) {
	setup := func(t *testing.T) spacemeshv2beta1.RewardStreamServiceClient {
		db := statesql.InMemoryTest(t)
		...
		conn := dialGrpc(t, cfg)
		return spacemeshv2beta1.NewRewardStreamServiceClient(conn)
	}
	...
	t.Run("watch", func(t *testing.T) {
		...
		for _, tc := range []struct {
		...
		}{
			{
				desc: "Smesher",
				request: &spacemeshv2beta1.RewardStreamRequest{
					FilterBy: &spacemeshv2beta1.RewardStreamRequest_Smesher{
			...
			},
			{
				desc: "Coinbase",
				request: &spacemeshv2beta1.RewardStreamRequest{
					FilterBy: &spacemeshv2beta1.RewardStreamRequest_Coinbase{
