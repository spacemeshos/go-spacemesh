...
func (s *TransactionStreamService) RegisterService(server *grpc.Server) {
	spacemeshv2beta1.RegisterTransactionStreamServiceServer(server, s)
}

func (s *TransactionStreamService) RegisterHandlerService(mux *runtime.ServeMux) error {
	return spacemeshv2beta1.RegisterTransactionStreamServiceHandlerServer(context.Background(), mux, s)
}
...
func (s *TransactionService) RegisterService(server *grpc.Server) {
	spacemeshv2beta1.RegisterTransactionServiceServer(server, s)
}

func (s *TransactionService) RegisterHandlerService(mux *runtime.ServeMux) error {
	return spacemeshv2beta1.RegisterTransactionServiceHandlerServer(context.Background(), mux, s)
}
...
func (s *TransactionService) List(
	...
	rst := make([]*spacemeshv2beta1.TransactionResponse, 0, request.Limit)
	...
	return &spacemeshv2beta1.TransactionList{Transactions: rst}, nil
}
...
func (s *TransactionService) ParseTransaction(
	...
	t := &spacemeshv2beta1.Transaction{
		Raw: raw.Raw,
	}
	...
	if header != nil {
		...
		t.Nonce = &spacemeshv2beta1.Nonce{Counter: header.Nonce}
	...
	return &spacemeshv2beta1.ParseTransactionResponse{
		Tx: t,
	}, nil
}
...
func (s *TransactionService) SubmitTransaction(
	...
	return &spacemeshv2beta1.SubmitTransactionResponse{
...
func (s *TransactionService) toTx(
	...
	rst := &spacemeshv2beta1.TransactionResponse{}
	...
	if tx.TxHeader != nil {
		...
		t.Nonce = &spacemeshv2beta1.Nonce{Counter: tx.Nonce}
		t.MaxGas = tx.MaxGas
	...
	if includeResult && result != nil {
		rst.TxResult = &spacemeshv2beta1.TransactionResult{
...
func (s *TransactionService) convertTxResult(
	...
	case types.TransactionSuccess:
		return spacemeshv2beta1.TransactionResult_TRANSACTION_STATUS_SUCCESS
	case types.TransactionFailure:
		return spacemeshv2beta1.TransactionResult_TRANSACTION_STATUS_FAILURE
	default:
		return spacemeshv2beta1.TransactionResult_TRANSACTION_STATUS_UNSPECIFIED
	}
}
...
func (s *TransactionService) convertTxState(
	...
	switch tx.State {
	case types.MEMPOOL:
		state := spacemeshv2beta1.TransactionState_TRANSACTION_STATE_MEMPOOL
		return &state
	case types.APPLIED:
		state := spacemeshv2beta1.TransactionState_TRANSACTION_STATE_PROCESSED
		return &state
	default:
		...
		if err != nil {
			...
			state := spacemeshv2beta1.TransactionState_TRANSACTION_STATE_UNSPECIFIED
			return &state
		}
		if evicted {
			state := spacemeshv2beta1.TransactionState_TRANSACTION_STATE_INEFFECTUAL
			return &state
		}
		state := spacemeshv2beta1.TransactionState_TRANSACTION_STATE_UNSPECIFIED
		return &state
...
func toTxContents(rawTx []byte) (*spacemeshv2beta1.TransactionContents,
	...
	res := &spacemeshv2beta1.TransactionContents{}
	txType := spacemeshv2beta1.Transaction_TRANSACTION_TYPE_UNSPECIFIED
	...
	case core.MethodSpawn:
		...
		case wallet.TemplateAddress:
			...
			res.Contents = &spacemeshv2beta1.TransactionContents_SingleSigSpawn{
				SingleSigSpawn: &spacemeshv2beta1.ContentsSingleSigSpawn{
					Pubkey: args.PublicKey.String(),
				},
			}
			txType = spacemeshv2beta1.Transaction_TRANSACTION_TYPE_SINGLE_SIG_SPAWN
		case multisig.TemplateAddress:
			args := txArgs.(*multisig.SpawnArguments)
			contents := &spacemeshv2beta1.TransactionContents_MultiSigSpawn{
				MultiSigSpawn: &spacemeshv2beta1.ContentsMultiSigSpawn{
			...
			txType = spacemeshv2beta1.Transaction_TRANSACTION_TYPE_MULTI_SIG_SPAWN
		case vesting.TemplateAddress:
			args := txArgs.(*multisig.SpawnArguments)
			contents := &spacemeshv2beta1.TransactionContents_VestingSpawn{
				VestingSpawn: &spacemeshv2beta1.ContentsMultiSigSpawn{
					Required: uint32(args.Required),
				},
			}
			...
			res.Contents = contents
			txType = spacemeshv2beta1.Transaction_TRANSACTION_TYPE_VESTING_SPAWN
		case vault.TemplateAddress:
			args := txArgs.(*vault.SpawnArguments)
			res.Contents = &spacemeshv2beta1.TransactionContents_VaultSpawn{
				VaultSpawn: &spacemeshv2beta1.ContentsVaultSpawn{
			...
			txType = spacemeshv2beta1.Transaction_TRANSACTION_TYPE_VAULT_SPAWN
		}
	case core.MethodSpend:
		args := txArgs.(*wallet.SpendArguments)
		res.Contents = &spacemeshv2beta1.TransactionContents_Send{
			Send: &spacemeshv2beta1.ContentsSend{
				Destination: args.Destination.String(),
				Amount:      args.Amount,
			},
		}
		txType = spacemeshv2beta1.Transaction_TRANSACTION_TYPE_SINGLE_SIG_SEND
		if r.Len() > types.EdSignatureSize {
			txType = spacemeshv2beta1.Transaction_TRANSACTION_TYPE_MULTI_SIG_SEND
		}
	case vesting.MethodDrainVault:
		args := txArgs.(*vesting.DrainVaultArguments)
		res.Contents = &spacemeshv2beta1.TransactionContents_DrainVault{
			DrainVault: &spacemeshv2beta1.ContentsDrainVault{
				Vault:       args.Vault.String(),
				Destination: args.Destination.String(),
				Amount:      args.Amount,
			},
		}
		txType = spacemeshv2beta1.Transaction_TRANSACTION_TYPE_DRAIN_VAULT
	}
