...
func (s SpacemeshGrpcService) GetNodeStatus(context.Context, *empty.Empty) (*pb.NodeStatus, error) {
	return &pb.NodeStatus{
		...
		SyncedLayer:   uint64(s.Tx.LatestLayer().Uint32()),
		CurrentLayer:  uint64(s.GenTime.GetCurrentLayer().Uint32()),
		VerifiedLayer: uint64(s.Tx.LatestLayerInState().Uint32()),
	}, nil
}
...
func (s SpacemeshGrpcService) GetAccountTxs(ctx context.Context, txsSinceLayer *pb.GetTxsSinceLayer) (*pb.AccountTxs, error) {
	...
	txs := pb.AccountTxs{ValidatedLayer: uint64(currentPBase.Uint32())}
