...
func init() {
	register("athena", athena())
}

func athena() config.Config {
	...
	defaultdir := filepath.Join(home, "athena-testnet", "/")
	return config.Config{
		...
		Genesis: config.GenesisConfig{
			...
			Accounts: map[string]uint64{
				wallet.TemplateAddress.StringWithHRP("atest"): 0,
			},
			Templates: map[string][]byte{
				wallet.TemplateAddress.StringWithHRP("atest"): wallet.PROGRAM,
			},
		},
		...
		Bootstrap: bootstrap.Config{
			URL:      "https://bootstrap.spacemesh.network/athenatestnet01",
