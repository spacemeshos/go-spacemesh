...
func TestGetLayerFromPeersTimeout(t *testing.T) {
	r := require.New(t)
	mf := newMockFetcher()
	lg := logtest.New(t).WithName("syncer")
	msh := newMemMesh(lg)
	...
	r.NoError(msh.AddZeroBlockLayer(types.NewLayerID(1)))
	syncer := NewSyncer(context.TODO(), conf, ticker, msh, mf, lg)

	// make sure we can get layer data
	mf.feedLayerResult(types.NewLayerID(1), types.NewLayerID(2))
	doneChan := make(chan struct{})
	go func() {
		_, err := syncer.getLayerFromPeers(context.TODO(), types.NewLayerID(1))
		r.NoError(err)
		doneChan <- struct{}{}
	}()
	select {
	...
	case <-time.After(time.Second):
		r.Fail("timed out waiting for getLayerFromPeers")
	}
	...
	select {
	case <-doneChan2:
		r.Fail("expected getLayerFromPeers to block")
