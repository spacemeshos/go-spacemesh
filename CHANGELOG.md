...
nodes it takes ~40 minutes. Vacuum takes ~5 minutes. The vacuum operation requires extra disk space
to complete successfully. If the size of state.sql is 25 GiB at the beginning of upgrade, the WAL file
(state.sql-wal) will grow to 25 GiB and then drop to 0 when the vacuum is complete. The final size
of state.sql is expected to be ~1.5 GiB.
