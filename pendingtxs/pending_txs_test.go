...
func TestNewAccountPendingTxs(t *testing.T) {
	...
	// Adding works
	pendingTxs.Add(types.LayerIDFromUint32(1), newTx(t, 5, 100, 1))
	empty = pendingTxs.IsEmpty()
	r.False(empty)
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	...
	pendingTxs.Add(types.LayerIDFromUint32(2), newTx(t, 5, 100, 1))
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	r.Equal(prevNonce+1, nonce)
	r.Equal(prevBalance-100, balance)

	// When it's added again in a lower layer -- nothing changes
	pendingTxs.Add(types.LayerIDFromUint32(0), newTx(t, 5, 100, 1))
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	r.Equal(prevNonce+1, nonce)
	r.Equal(prevBalance-100, balance)
	...
	pendingTxs.Add(types.LayerIDFromUint32(1),
		newTx(t, 5, 100, 2),
		newTx(t, 5, 50, 1),
	)
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	...
	pendingTxs.Add(types.LayerIDFromUint32(1), newTx(t, 6, 950, 1))
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
	...
	pendingTxs.Add(types.LayerIDFromUint32(1), newTx(t, 6, 50, 1))
	nonce, balance = pendingTxs.GetProjection(prevNonce, prevBalance)
