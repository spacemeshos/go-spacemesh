...
func TestNewEdSignerFromBuffer(t *testing.T) {
	b := []byte{1, 2, 3}
	_, err := NewEdSignerFromBuffer(b, [20]byte{})
	assert.NotNil(t, err)
...
func TestEdSigner_Sign(t *testing.T) {
	genesisID := [20]byte{}
	ed := NewEdSigner(genesisID)
	m := make([]byte, 4)
	rand.Read(m)
	sig := ed.Sign(m)
	assert.True(t, ed25519.Verify2(ed25519.PublicKey(ed.PublicKey().Bytes()), append(genesisID[:], m...), sig))
}

func TestNewEdSigner(t *testing.T) {
	ed := NewEdSigner([20]byte{})
...
func TestEdSigner_ToBuffer(t *testing.T) {
	...
	ed := NewEdSigner(genesisID)
	buff := ed.ToBuffer()
	ed2, err := NewEdSignerFromBuffer(buff, genesisID)
	assert.Nil(t, err)
...
func TestEdVerifierWithGenesisID_Verify(t *testing.T) {
	...
	edSigner := NewEdSigner(genesisID)
	verifier := NewEdVerifier(genesisID)
	...
	t.Run("Fail - short msg", func(t *testing.T) {
		...
		r.Equal(false, verifier.Verify(edSigner.PublicKey(), msg, sign))
	})
	...
	t.Run("Fail - wrong msg", func(t *testing.T) {
		...
		r.Equal(false, verifier.Verify(edSigner.PublicKey(), append(genesisID[:], msg...), sign))
	})
	...
	t.Run("Success", func(t *testing.T) {
		...
		r.Equal(true, verifier.Verify(edSigner.PublicKey(), append(genesisID[:], msg...), sign))
	})
}
