...
func (mt *multiPeerSyncTester) expectProbe(times int, pr rangesync.ProbeResult) *peerList {
	var pl peerList
	mt.syncBase.EXPECT().Probe(gomock.Any()).DoAndReturn(
		func(p p2p.Peer) (rangesync.ProbeResult, error) {
...
func (mt *multiPeerSyncTester) expectFullSync(pl *peerList, times, numFails int) {
	...
	mt.syncBase.EXPECT().Derive(gomock.Any()).DoAndReturn(func(p p2p.Peer) Syncer {
		...
		expSync := s.EXPECT().Sync(gomock.Nil(), gomock.Nil())
		if numFails != 0 {
...
func TestMultiPeerSync(t *testing.T) {
	...
	t.Run("probe failure", func(t *testing.T) {
		...
		mt.addPeers(10)
		mt.syncBase.EXPECT().Count().Return(100, nil).AnyTimes()
		mt.syncBase.EXPECT().Probe(gomock.Any()).
