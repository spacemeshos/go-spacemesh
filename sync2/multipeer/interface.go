...
type OrderedSet interface {
	...
	// or advanced.
	Advance() error
	// Added returns the sequence of items that were received set since the original
	// set was copied. It is not guaranteed that the items are absent in the original
	// set. If you make a copy of copy, the 2nd copy's Added sequence may include the
	// added items from 1st copy.
...
type SyncBase interface {
	...
	Probe(p p2p.Peer) (rangesync.ProbeResult, error)
...
type Syncer interface {
	...
	Sync(x, y rangesync.KeyBytes) error
	// Serve serves a synchronization request on the specified stream.
	Serve(stream io.ReadWriter) error
...
type SyncKeyHandler interface {
	...
	Commit(peer p2p.Peer, new OrderedSet) error
}
...
type PairwiseSyncer interface {
	...
	Serve(stream io.ReadWriter, os rangesync.OrderedSet) error
}
