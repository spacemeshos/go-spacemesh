...
func (ssb *SetSyncBase) Count() (int, error) {
	...
	info, err := ssb.os.GetRangeInfo(x, x, -1)
	if err != nil {
		return 0, err
	}
...
func (ssb *SetSyncBase) Probe(p p2p.Peer) (rangesync.ProbeResult, error) {
	...
	pr, err := ssb.ps.Probe(p, os, nil, nil)
	if err != nil {
...
func (ss *setSyncer) Sync(x, y rangesync.KeyBytes) error {
	if err := ss.ps.Sync(ss.p, ss, x, y); err != nil {
		return err
	}
	return ss.commit()
}
...
func (ss *setSyncer) Serve(stream io.ReadWriter) error {
	if err := ss.ps.Serve(stream, ss); err != nil {
		return err
	}
	return ss.commit()
}
...
func (ss *setSyncer) commit() error {
	if err := ss.handler.Commit(ss.p, ss.OrderedSet); err != nil {
		return err
	}
