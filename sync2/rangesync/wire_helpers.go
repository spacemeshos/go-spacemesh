...
func (c *CompactHash) ToOrdered() types.Ordered {
	if c.H == nil {
		return nil
	}
	return c.H
...
func KeyBytesToCompact(h types.KeyBytes) CompactHash {
	return CompactHash{H: h.Clone()}
}

func OrderedToCompactHash(h types.Ordered) CompactHash {
	return CompactHash{H: h.(types.KeyBytes).Clone()}
}
