...
func naiveRange(all []types.KeyBytes, x, y types.KeyBytes, stopCount int) (items []types.KeyBytes, startID, endID types.KeyBytes) {
	if len(all) == 0 {
		return nil, nil, nil
	}
	// all = slices.Clone(all) // QQQQQ: should not need this
	// slices.Sort(all)        // QQQQQ: should not need this
	start := gtePos(all, x)
	end := gtePos(all, y)
	if x.Compare(y) < 0 {
...
func (ds *dumbSet) seq(n int) types.Seq {
	...
	return types.Seq(func(yield func(types.Ordered, error) bool) {
...
func (ds *dumbSet) getRangeInfo(
	...
	vx := x.(types.KeyBytes)
	vy := y.(types.KeyBytes)
	rangeItems, start, end := naiveRange(ds.keys, vx, vy, count)
	fpFunc := ds.fpFunc
	if fpFunc == nil {
		fpFunc = naiveFPFunc
	}
...
func (ds *dumbSet) Has(ctx context.Context, k types.Ordered) (bool, error) {
