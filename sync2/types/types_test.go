...
var fakeSeq Seq = func(yield func(Ordered, error) bool) {
	items := []KeyBytes{
		{1},
		{2},
		{3},
...
var fakeErrSeq Seq = func(yield func(Ordered, error) bool) {
	items := []KeyBytes{
		{1},
		{2},
	}
...
var fakeErrOnlySeq Seq = func(yield func(Ordered, error) bool) {
	yield(nil, fakeErr)
}
...
func TestGetN(t *testing.T) {
	actual, err := GetN[KeyBytes](fakeSeq, 2)
	require.NoError(t, err)
	require.Equal(t, []KeyBytes{{1}, {2}}, actual)
	actual, err = GetN[KeyBytes](fakeSeq, 5)
	require.NoError(t, err)
	require.Equal(t, []KeyBytes{{1}, {2}, {3}, {4}, {1}}, actual)
	actual, err = GetN[KeyBytes](fakeErrSeq, 2)
	require.NoError(t, err)
	require.Equal(t, []KeyBytes{{1}, {2}}, actual)
	actual, err = GetN[KeyBytes](fakeErrSeq, 5)
	require.Equal(t, fakeErr, err)
	require.Nil(t, actual)
	actual, err = GetN[KeyBytes](fakeErrOnlySeq, 2)
	require.Equal(t, fakeErr, err)
	require.Nil(t, actual)
}
